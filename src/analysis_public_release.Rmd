...
# mesenchymal vs neural crest markers
 ```{r mesenchymal.neural_crest}

genes = DGEList(counts=geneCounts)
genes = calcNormFactors(genes)
geneExprlog2RPKM_all = rpkm( genes, geneInfo$Length, log=TRUE )

# 'VTN'
geneList = list(Mesenchymal=c('NT5E', 'VIM', 'THBS1', 'CDH2',  'FN1', 'ENG', 'ITGB1', 'CD44', 'THY1'), 
  "Neural crest" = c('NGFR', 'TFAP2A', 'NR2F1', 'NR2F2', 'TWIST1', 'SNAI1', 'SNAI2', 'RARA', 'ALX3', 'ALX4', 'PAX3', 'SOX9', 'SOX10', 'MYC', 'SEMA3A', 'NOTCH1', 'NOTCH2', 'ASCL1', 'CHD7', 'FOXD3', 'NEUROG1', 'NEUROG2', 'NEUROG3', 'NUMB', 'VIM', 'BMP4', 'BMP7')) 

for(key in names(geneList)){

  # Check Fibroblast signature, NPC and Neurons for CD73 (NT5E) and CD271 (NGFR)
  geneMarker = sort(geneList[[key]])
  ensGenes = with(geneInfo, Geneid[match(geneMarker,geneName ) ])

  df2 = t(geneExprlog2RPKM_all[ensGenes,,drop=FALSE])
  colnames(df2) = geneMarker

  df2 = melt( data.frame(ID = colnames(geneExprlog2RPKM_all), df2, CellType=metadata$Cell.Type)) 
  ...
  figList[[key]] = ggplot(df_combined, aes(Class, value)) + geom_boxplot(aes(fill=fill)) + geom_point(size=1) + theme_bw() + ylab(bquote(log[2]~RPKM)) + xlab('') + facet_wrap(~Var2, ncol=5) + coord_flip()  + scale_fill_manual(values = c("green2", "orange", "white") ) + theme(legend.position="none") + ggtitle(key) + theme(plot.title = element_text(hjust = 0.5)) + ylim(c(-8, 11))
}
```

```{r mesenchymal, cache=FALSE, fig.height=4, fig.width=8} 
figList[['Mesenchymal']]
```


```{r neural_crest, cache=FALSE, fig.height=12, fig.width=8} 
figList[["Neural crest"]]
```
