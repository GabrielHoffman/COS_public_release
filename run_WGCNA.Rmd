...
geneInfo = read.csv(getFileLocation(synGet( 'syn9909914' )), header=TRUE, stringsAsFactors=FALSE)
...
for( cellType in names(colorDynamicTOM) ){
  ...
  file = paste0("Coexpression_modules_", cellType, ".tsv")
...
geneSetsSubet[["COS_DE"]] = with(resDEall, symbol[qvalue < 0.10])
...
geneSetsSubet[['SendaiDE']] = with(resDENorm, symbol[qvalue < 0.05])
...
modulesInclude = sapply(df$cell_Module, function(module){
    min(df$Pvalue[as.character(df$cell_Module) == module]) < 0.05/nrow(df)
  })
...
```{r coexpression.plot, fig.width=30}
...
gs_order2 = c(gs_order2, gs_order[grep("DE", gs_order)][-3])
...
df3$Gene.Set = factor(df3$Gene.Set, gs_order2)
...
```{r coexpression.plot2, fig.width=30, cache=FALSE}

ggplot(df3, aes(Gene.Set, cell_Module, fill=pmin(-log10(Pvalue), 20))) + geom_tile(color="white") + coord_flip() +
 scale_fill_gradientn(values=c(0, .1, .5, .99999, 1), colours=c("white", "orange", "red", "firebrick", "black"), 
  name =bquote(-log[10]~P)) +ylab("Coexpression module") + theme_bw() +  xlab("Gene set") + 
  theme(axis.text.y = element_text(angle = 0, hjust = 1),
      axis.text.x = element_blank(),panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), aspect.ratio=ratio) 
...
```{r coexpression.plot3, fig.width=30, cache=FALSE}

ggplot(df3, aes(Gene.Set, cell_Module, fill=color)) + geom_tile(color="white") + coord_flip() +
 scale_fill_manual(values=colors, name =bquote(-log[10]~P)) + 
  ylab("Coexpression module") + theme_bw(10) +  xlab("Gene set") + 
  theme(axis.text.y = element_text(angle = 0, hjust = 1),
      axis.text.x = element_blank(),panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), aspect.ratio=ratio) 
...
# color modules
```{r moduleColors, fig.width=20, cache=FALSE}
...
coexprDE = foreach( cellType = levels(metadata$Cell.Type) ) %do% {
 ...
 EVect[[cellType]] = foreach( module = modulesArray, .combine=rbind ) %do% {
  ...
  idx = match(colnames(EVect[[cellType]]), metadata2$Sample.Name)

  design = model.matrix(~ Dx , metadata2[idx,])
 
  fit = lmFit( EVect[[cellType]], design, block=metadata2$Donor[idx], correlation=dupcor$consensus )
  fit = eBayes(fit)
