...
fig2 = plot_concordance_logFC( df ) + ylab("logFC from HBCC") + ylab("logFC from CommonMind") + ggtitle("HBCC vs CommonMind")    

grid.arrange( fig1, fig2, ncol=2)   
```

## Current dataset (COS) compared to CMC and HBCC
...
corValues = with(df, cor.test(t.x, t.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("CommonMind", "COS/NPC", "t", corValues$estimate,  corValues$p.value))

corValues = with(df, cor.test(logFC.x, logFC.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("CommonMind", "COS/NPC", "logFC", corValues$estimate,  corValues$p.value))

fig1 = plot_concordance( df ) + ylab("t-statistic from COS/NPC") + xlab("t-statistic from CommonMind") + ggtitle("COS/NPC vs CommonMind")  
fig2 = plot_concordance_logFC( df ) + ylab("logFC from  COS/NPC") + ylab("logFC from CommonMind") + ggtitle("COS/NPC vs CommonMind")    

df = merge(CMCDE_HBCC, resDEall_Scz_NPC,by.x="genes", by.y="row.names")

corValues = with(df, cor.test(t.x, t.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("HBCC", "COS/NPC", "t", corValues$estimate,  corValues$p.value))

corValues = with(df, cor.test(logFC.x, logFC.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("HBCC", "COS/NPC", "logFC", corValues$estimate,  corValues$p.value))

fig3 = plot_concordance( df ) + ylab("t-statistic from COS/NPC") + xlab("t-statistic from HBCC") + ggtitle("COS/NPC vs HBCC")  
fig4 = plot_concordance_logFC( df ) + ylab("logFC from  COS/NPC") + ylab("logFC from HBCC") + ggtitle("COS/NPC vs HBCC")  
...
corValues = with(df, cor.test(t.x, t.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("CommonMind", "COS/Neuron", "t", corValues$estimate,  corValues$p.value))

corValues = with(df, cor.test(logFC.x, logFC.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("CommonMind", "COS/Neuron", "logFC", corValues$estimate,  corValues$p.value))


fig1 = plot_concordance( df ) + ylab("t-statistic from COS/Neuron") + xlab("t-statistic from CommonMind") + ggtitle("COS/Neuron vs CommonMind")  
fig2 = plot_concordance_logFC( df ) + ylab("logFC from  COS/Neuron") + ylab("logFC from CommonMind") + ggtitle("COS/Neuron vs CommonMind")    


df = merge(CMCDE_HBCC, resDEall_Scz_Neuron,by.x="genes", by.y="row.names")

corValues = with(df, cor.test(t.x, t.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("HBCC", "COS/Neuron", "t", corValues$estimate,  corValues$p.value))

corValues = with(df, cor.test(logFC.x, logFC.y, method="spearman"))
concordanceDF = rbind(concordanceDF, c("HBCC", "COS/Neuron", "logFC", corValues$estimate,  corValues$p.value))

fig3 = plot_concordance( df ) + ylab("t-statistic from COS/Neuron") + xlab("t-statistic from HBCC") + ggtitle("COS/Neuron vs HBCC")  
fig4 = plot_concordance_logFC( df ) + ylab("logFC from  COS/Neuron") + ylab("logFC from HBCC") + ggtitle("COS/Neuron vs HBCC")    
...
## Concordance between t-statistics 
```{r combined.t}
statistic = "t"
...
for(covariate in c('NULL',colnames(cibersort)) ){
  ...
  df = merge(CMCDE, resDEall_Scz_NPC,by.x="genes", by.y="row.names")
  corValues = with(df, cor.test(t.x, t.y, method="spearman"))
  concordanceDFall = rbind(concordanceDFall, c("CommonMind", "COS/NPC", covariate, "t", corValues$estimate,  corValues$p.value))
  fig1 = plot_concordance( df ) + ylab("t-statistic from COS/NPC") + xlab("t-statistic from CommonMind") + ggtitle("COS/NPC vs CommonMind")  

  df = merge(CMCDE_HBCC, resDEall_Scz_NPC,by.x="genes", by.y="row.names")
  corValues = with(df, cor.test(t.x, t.y, method="spearman"))
  concordanceDFall = rbind(concordanceDFall, c("HBCC", "COS/NPC", covariate, "t", corValues$estimate,  corValues$p.value))
  fig2 = plot_concordance( df ) + ylab("t-statistic from COS/NPC") + xlab("t-statistic from HBCC") + ggtitle("COS/NPC vs HBCC")  

  # Neuron
  df = merge(CMCDE, resDEall_Scz_Neuron,by.x="genes", by.y="row.names")
  corValues = with(df, cor.test(t.x, t.y, method="spearman"))
  concordanceDFall = rbind(concordanceDFall, c("CommonMind", "COS/Neuron", covariate, "t", corValues$estimate,  corValues$p.value))
  fig3 = plot_concordance( df ) + ylab("t-statistic from COS/Neuron") + xlab("t-statistic from CommonMind") + ggtitle("COS/Neuron vs CommonMind")  

  df = merge(CMCDE_HBCC, resDEall_Scz_Neuron,by.x="genes", by.y="row.names")
  corValues = with(df, cor.test(t.x, t.y, method="spearman"))
  concordanceDFall = rbind(concordanceDFall, c("HBCC", "COS/Neuron", covariate, "t", corValues$estimate,  corValues$p.value))
  fig4 = plot_concordance( df ) + ylab("t-statistic from COS/Neuron") + xlab("t-statistic from HBCC") + ggtitle("COS/Neuron vs HBCC")  

  # grid.arrange( fig1, fig3, fig2, fig4, ncol=2) 
}
```

```{r plot.concordance.rho, fig.width=14}
...
```{r plot.concordance.pvalue, fig.width=14}
statistic = "t"   

fig1 = ggplot(subset(concordanceDFall, stat==statistic & dataset1=='CommonMind'), aes(covariate, -log10(pvalue), fill=dataset2)) + geom_bar(stat='identity', position=position_dodge()) +  xlab('')  + ylab(bquote(-log[10]~p-value)) +
  guides(fill=guide_legend(title="Post mortem datasets")) + theme_bw(16) +
  theme(plot.title=element_text(hjust=0.5), legend.position="none", aspect.ratio=2) +
  coord_flip() + scale_fill_manual(values=collocal) + ggtitle("Concordance with CommonMind")

fig2 = ggplot(subset(concordanceDFall, stat==statistic & dataset1=='HBCC'), aes(covariate,  -log10(pvalue), fill=dataset2)) + geom_bar(stat='identity', position=position_dodge()) +  xlab('') + ylab(bquote(-log[10]~p-value)) +
  guides(fill=guide_legend(title="Post mortem datasets")) + theme_bw(16) +
  theme(plot.title=element_text(hjust=0.5), legend.position="none", aspect.ratio=2) +
  coord_flip() + scale_fill_manual(values=collocal) + ggtitle("Concordance with HBCC")
