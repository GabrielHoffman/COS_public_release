...
cutoff2 = 0.01
df = resDE_cellType
col = rev(c("> 5%"="grey60", "< 5%"="firebrick", "< 1%"="red"))
...
# Enrichments for gense up in Neurons
...
# Enrichments for gense up in NPCs
...
```{r cibersort.disease, fig.width=12, fig.height=5}
...
ggplot(resdf, aes(score, -log10(pvalue), fill=CellType)) + geom_bar(stat='identity', position=position_dodge()) + theme_bw(15) + coord_flip() + scale_fill_manual(values=c("orange", "green2")) + xlab('')
...
# Differential expression between cases and controls
...
ggplot(df2, aes(logFC.x, logFC.y)) + geom_errorbar(aes(ymax = logFC.y + se_beta.y, ymin=logFC.y - se_beta.y), color="grey") +
 ...
 geom_point(aes(color=color)) + theme_bw(15) +
  scale_color_manual(values = array(col), name="FDR") +  xlab("logFC in Neurons") + ylab("logFC in NPCs")  + 
...
ggplot(df2, aes(t.x, t.y)) + geom_point(aes(color=color)) + theme_bw(15) +
  scale_color_manual(values = array(col), name="FDR") + xlab("t-statistic in Neurons") + ylab("t-statistic in NPCs")  + 
  geom_text_repel(data=df2[df2$qvalue <cutoff2,], aes(label=gene),  min.segment.length = unit(1, "lines")) + 
