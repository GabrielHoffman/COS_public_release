...
```{r PCA.residuals, cache=FALSE, fig.height=5}
...
```{r within_between.residuals, echo=FALSE, message=FALSE, cache=FALSE, fig.height=5}     
simList = list()
y_range = c() 
for( cellType in rev(levels( metadata2$Cell.Type)) ){
...
genes = c( "SNX19", 'FURIN', 'TSNARE1', 'CNTN4', 'CLCN3', 'SNAP91', 'FZD6', 'SLITRK2', 'PPP1R12B', "BCAN", 'PRRX1', 'SMARCE1', "UTY", 'STEAP3')
...
```{r Fig.variancePartition, fig.width=12, cache=FALSE}
     
layout = matrix(c(1,2,2,2,3,4,5,6), ncol = 4, byrow = TRUE)   
 fig = figPercent+ theme(legend.position="top")
grid.arrange(fig, figVp, figStratify_CNTN4, figStratify_FZD6, figStratify_SLITRK2, figStratify_PRRX1, layout_matrix = layout, heights=c(1,1), padding = unit(0.05, "line"))      
```

```{r Fig.variancePartition1, fig.width=6, cache=FALSE}     
...
### Genes with FDR < 20%
```{r DE.genes}
kable(resDEall_Scz[resDEall_Scz$qvalue < .20,c(1:4, 7, 8)])
```
...
cutoff = 0.20
cutoff2 = 0.10
col = rev(c("n.s."="grey60", "< 20%"="firebrick", "< 10%"="red"))
...
ggplot(df2, aes(logFC.x, logFC.y)) + geom_errorbar(aes(ymax = logFC.y + se_beta.y, ymin=logFC.y - se_beta.y), color="grey") +
 ...
 geom_point(aes(color=color)) + theme_bw(15) +
  scale_color_manual(values = array(col), name="FDR") +  
  xlab( bquote(log[2]~fold~change~ln~neurons)) + ylab(bquote(log[2]~fold~change~ln~NPCs))  + 
  geom_text_repel(data=df2[df2$qvalue <cutoff2,], aes(label=gene),  min.segment.length = unit(1, "lines")) + 
  theme(legend.position=c(.1,.9), aspect.ratio=1) + geom_abline(color="grey") + xlim(range(xlim, ylim)) + ylim(range(xlim, ylim)) 


lim = range(c( with(df2, t.x), with(df2, t.y)))
ggplot(df2, aes(t.x, t.y)) + geom_point(aes(color=color)) + theme_bw(15) +
  scale_color_manual(values = array(col), name="FDR") + 
  xlab( bquote(t-statistic~ln~neurons)) + ylab(bquote(t-statistic~ln~NPCs)) +
  geom_text_repel(data=df2[df2$qvalue <cutoff2,], aes(label=gene),  min.segment.length = unit(1, "lines")) + 
  theme(legend.position=c(.1,.9), aspect.ratio=1) + geom_abline(color="grey") + xlim(lim) + ylim(lim)
```
...
kable(res[res$qvalue < 0.2,c(1:4, 7, 8)])
```
...
do.call("grid.arrange", list(fig1, fig2, ncol=2))
```
...
```{r compare.t.NPC.enrichment.up.enrich, fig.height=4.5, fig.width=9, cache=FALSE}
res = resEnrichUp[1:10,]
res = clean_names( res )
res$`Gene Set` = factor( res$`Gene Set`,  rev(res$`Gene Set`))

fig1 = ggplot(res, aes(x=`Gene Set`, y=-log10(Pvalue))) + geom_bar(stat='identity', fill="red") + 
...
fig2 = ggplot(res, aes(x=`Gene Set`, y=-log10(Pvalue))) + geom_bar(stat='identity', fill="blue") + 
  coord_flip() + theme_bw(8) + ylab(bquote(Enrichment~(-log[10]~p))) + xlab('')
...
```{r compare.t.neuron.enrichment.up.enrich, fig.height=3, fig.width=9}
res = resEnrichUp[1:10,]
res = clean_names( res )
res$`Gene Set` = factor( res$`Gene Set`, rev(res$`Gene Set`))
